...
class DocumentClassifier:
    def __init__(self, model_path, config_path=None, token=None):
        # Đọc candidate labels từ file cấu hình nếu có
        self.candidate_labels = []
        if config_path and os.path.exists(config_path):
            with open(config_path, "r", encoding="utf-8") as f:
                config = json.load(f)
                categories = config.get("categories", {})
                # Danh sách candidate labels được lấy từ các giá trị trong categories
        ...
        # Khởi tạo tokenizer (bạn có thể sử dụng fast hoặc slow tokenizer tùy theo yêu cầu)
    ...
    def classify(self, text):
        # Nếu không có candidate labels, trả về "Unknown"
        if not self.candidate_labels:
            return "Unknown", 0.0
